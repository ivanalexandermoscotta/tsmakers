...
class BoundaryTimespanMaker(timespantools.TimespanMaker):
    r'''A boundary timespan-maker.

    ::

        >>> timespan_maker = timespantools.BoundaryTimespanMaker(
        ...
        timespantools.BoundaryTimespanMaker(
            start_talea=rmakers.Talea(
                counts=[1],
                denominator=2,
                ),
    ...
    def _collect_preexisting_timespans(
        ...
        for timespan in timespan_list:
            assert isinstance(timespan, (
                timespantools.PerformedTimespan,
                timespantools.SilentTimespan,
                ))
            if isinstance(timespan, timespantools.SilentTimespan):
                continue
            if (
