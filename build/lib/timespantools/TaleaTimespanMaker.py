...
class TaleaTimespanMaker(timespantools.TimespanMaker):
    r'''A talea timespan maker.

    ::

        >>> timespan_maker = timespantools.TaleaTimespanMaker(
        ...
        timespantools.tools.TaleaTimespanMaker(
            initial_silence_talea=rmakers.Talea(
                counts=[0, 4],
                denominator=16,
                ),
    ...
    ::
        ...
        abjad.TimespanList(
            [
                timespantools.PerformedTimespan(
    ...
    def __init__(
        ...
        timespantools.TimespanMaker.__init__(
            self,
    ...
    def _make_timespans(
        ...
        initial_silence_talea = timespantools.Cursor(initial_silence_talea)
        playing_talea = timespantools.Cursor(self.playing_talea)
        playing_groupings = timespantools.Cursor(self.playing_groupings)
        silence_talea = self.silence_talea
        if silence_talea is None:
            silence_talea = rmakers.Talea(counts=(0,), denominator=1)
        silence_talea = timespantools.Cursor(silence_talea)
    ...
    def _make_with_synchronized_step(
        ...
        while start_offset < stop_offset and can_continue:
            ...
            for context_name, music_specifier in music_specifiers.items():
                ...
                if all(isinstance(_, timespantools.SilentTimespan)
