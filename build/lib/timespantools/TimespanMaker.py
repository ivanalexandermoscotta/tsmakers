...
class TimespanMaker():
    ...
    def __init__(
        ...
        if timespan_specifier is not None:
            assert isinstance(timespan_specifier, timespantools.TimespanSpecifier)
    ...
    def _coerce_music_specifiers(music_specifiers):
        ...
        prototype = (
            timespantools.MusicSpecifierSequence,
            timespantools.CompositeMusicSpecifier,
            )
        for context_name, music_specifier in music_specifiers.items():
            if music_specifier is None:
                music_specifier = [None]
            if not isinstance(music_specifier, prototype):
                music_specifier = timespantools.MusicSpecifierSequence(
